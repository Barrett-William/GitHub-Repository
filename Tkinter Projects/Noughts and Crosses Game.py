...
class GUI:
    ...
    def __init__(self, master):
        ...
        self.Canvas = tk.Canvas(self.frame, bg="white",
                                width=self.w, height=self.w)
        m_i = 3
        self.set(m_i)

        # Buttons
        self.quit_button = tk.Button(
            master, text="Quit", command=master.destroy).pack(side="right")
        self.label = tk.Label(
            master, text="Enter size of playing field:").pack(side="left")
        m_i = tk.IntVar(master, value=3)
        self.input_field = tk.Entry(master, font=(
            'arial', 18, 'bold'), textvariable=m_i, width=2, bg="white", bd=0).pack(side="left", padx=15)
        self.restart_button = tk.Button(
            master, text="Restart", command=lambda: self.reset(m_i.get())).pack(side="left")
        ...
        for i in rng:
            for j in rng:
                self.Canvas.create_line(
                    0, i*self.w/m_i, self.w, i*self.w/m_i, width=self.lw)
                self.Canvas.create_line(
                    j*self.w/m_i, 0, j*self.w/m_i, self.w, width=self.lw)
        self.Canvas.pack()
    ...
    def turn(self, event, m_i): # Click - turn counter and click recording
        self.t += 1
    ...
    def end(self, w_f):
        if w_f == 0:
            ...
            self.Canvas.create_text(
                250, 250, fill="blue", font="Helvetica 40 bold", text="Noughts wins!")
        elif w_f == 1:
            self.Canvas.create_rectangle(50, 200, 450, 300, fill="white")
            self.Canvas.create_text(
                250, 250, fill="red", font="Helvetica 40 bold", text="Crosses wins!")
        elif w_f == -1:
            self.Canvas.create_rectangle(50, 200, 450, 300, fill="white")
            self.Canvas.create_text(
